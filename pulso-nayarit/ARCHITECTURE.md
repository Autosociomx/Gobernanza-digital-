...
| Append-only | Reglas `allow update, delete: if false` | `REVOKE` + triggers que rechazan `UPDATE`/`DELETE`/`TRUNCATE` **para todos los roles de la API, incluido `service_role`** | En Firestore el Admin SDK ignora las reglas; aquí ni la conexión administrativa puede editar (verificado en despliegue) |
...
La única vía de escritura es la RPC `pulso_cast_vote` (`SECURITY DEFINER`), que valida que la consulta esté abierta, que la hora actual caiga dentro de la ventana `opens_at`/`closes_at` y que la opción exista, antes de insertar. Como recibo, devuelve el número de registro que el trigger de encadenado reportó para ese voto.
...
Alterar un registro histórico rompe la cadena hacia adelante y es detectable por **cualquier** auditor que recompute los hashes — la consulta exacta está en [`supabase/README.md`](./supabase/README.md). Además: los triggers `pulso_*_immutable` rechazan `UPDATE`/`DELETE`, guardias a nivel de sentencia bloquean `TRUNCATE` (que no dispara triggers de fila), y los tres roles de la API — `anon`, `authenticated` y `service_role` — tienen revocada la escritura directa sobre `votes` y `ledger`, de modo que ni siquiera la clave de servicio puede insertar registros forjados saltándose las validaciones de la RPC. Todo verificado en despliegue con conexión administrativa.
...
3. La vista `pulso_heatmap` aplica supresión k-anónima doble (k = 5): no se publica un código postal con menos de 5 votos totales, **ni ninguna celda (CP, opción) con menos de 5 votos** — sin la segunda condición, un CP de 5 votos con distribución 4–1 revelaría con certeza el voto del elector minoritario.
...
> Nota de capacidad del despliegue actual: el `pg_advisory_xact_lock` serializa los appends del ledger (~cientos de votos/s de techo). El lock es global **porque la cadena es global**: particionarlo por consulta exige primero particionar la cadena (un `prev_hash` por consulta, con génesis propio y verificación `partition by poll_id`). Ese es el diseño previsto para correr varias consultas estatales concurrentes; no es un cambio de una línea y por eso se hará como migración deliberada, no como ajuste rápido.
