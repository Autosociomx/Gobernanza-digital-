...
async function startServer() {
  ...
  // Triaje médico CIE-11: reemplaza Claude por Gemini. Misma lógica,
  // mismo formato JSON, cero dependencia de Anthropic.
  app.post("/api/ai/triage", async (req, res) => {
    const { sintomas, municipio } = req.body;

    if (!sintomas || sintomas.length < 5) {
      return res.status(400).json({
        error: "Por favor describe tus síntomas con más detalle."
      });
    }

    try {
      const ai = getAI();
      const triageSystemPrompt = `Eres ConectaX, el asistente de triaje médico digital del Estado de Nayarit, México.
...
FORMATO DE RESPUESTA (JSON exacto, sin comas finales):
{
  "nivel_urgencia": "ALTA o MEDIA o BAJA",
  "emoji": "🔴 o 🟡 o 🟢",
  "tiempo_espera": "estimado en minutos",
  ...
  "descripcion_cie11": "nombre de la condición en español según CIE-11"
      ...
      const response = await ai.models.generateContent({
        ...
        contents: [{
          role: "user",
          parts: [{
            text: `Municipio: ${municipio || "Tepic, Nayarit"}\nSíntomas: ${sintomas}`
          }]
        }],
        config: {
      ...
      console.error("ConectaX Triage error:", error);
      res.status(500).json({
