...
import {
  ...
  Wifi, Music, Download, Calendar, TrendingUp, Leaf, CheckCircle2
} from 'lucide-react';
import { cn } from '../../lib/utils';

type PatrimonioTab = 'lenguas' | 'programas' | 'mercado';
...
const LENGUAS = [
  {
    ...
    textColor: 'text-rose-400', bg: 'bg-rose-500/10', border: 'border-rose-500/20',
    bar: 'bg-rose-500',
  },
  {
    ...
    textColor: 'text-blue-400', bg: 'bg-blue-500/10', border: 'border-blue-500/20',
    bar: 'bg-blue-500',
  },
  {
    ...
    textColor: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20',
    bar: 'bg-emerald-500',
  },
];
...
const ARTESANOS = [
  { id: 1, nombre: 'María de los Ángeles H.', comunidad: 'La Yesca',  esp: 'Nierika · arte de estambre',  productos: 12, ventas: 48,  av: 'M', color: 'bg-rose-500'    },
  { id: 2, nombre: 'José Ramón B.',           comunidad: 'Mezquitic', esp: 'Chaquira y joyería',           productos: 8,  ventas: 31,  av: 'J', color: 'bg-amber-500'   },
  { id: 3, nombre: 'Lucía Carrillo T.',        comunidad: 'El Nayar',  esp: 'Textiles y bordados',          productos: 15, ventas: 67,  av: 'L', color: 'bg-purple-500'  },
  { id: 4, nombre: 'Cooperativa Wixárika',    comunidad: 'Tepic',     esp: 'Arte colectivo certificado',   productos: 34, ventas: 152, av: 'C', color: 'bg-blue-600'    },
];

const WEBINARS = [
  { titulo: 'Arte Wixárika: de la tradición al mercado digital', fecha: '15 Jul 2026', hora: '17:00', inscritos: 89  },
  { titulo: 'Cómo vender artesanía nayarita en todo México',     fecha: '22 Jul 2026', hora: '18:00', inscritos: 54  },
    ...
    <div className="space-y-6">
      <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-blue-500 mt-0.5">Audio · Vocabulario · Gramática · Lecciones interactivas · Asistente de voz</p>
        </div>
      </div>
        ...
        {LENGUAS.map(l => (
          <div key={l.nombre} className={cn('bg-[#161920] border rounded-xl p-5', l.border)}>
            ...
            <div className="h-1.5 bg-slate-800 rounded-full mb-4">
              <div className={cn('h-1.5 rounded-full transition-all', l.bar)} style={{ width: `${l.pct}%` }} />
            </div>
            ...
            <div className="flex gap-2">
              <button className={cn('flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-[10px] font-bold border transition-colors', l.border, l.textColor, 'hover:opacity-80')}>
      ...
      <div className="bg-[#161920] border border-slate-800 rounded-xl p-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          {[
            { num: '01', label: 'Grabación con hablantes nativos',      icon: Mic        },
            { num: '02', label: 'Transcripción y validación comunitaria', icon: CheckCircle2 },
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#161920] border border-slate-800 rounded-xl overflow-hidden">
        ...
        <div className="divide-y divide-slate-800/50">
          {PROGRAMAS.map(p => {
            const Icon = p.icon;
            const estadoColor = p.estado === 'activo' ? 'text-emerald-400 bg-emerald-500/10' :
      ...
      <div className="border border-dashed border-slate-700 rounded-xl p-5 text-center">
        ...
        <p className="text-xs text-slate-600 mb-3">Cualquier comunidad originaria puede proponer nuevos programas a través del módulo de participación ciudadana.</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-bold text-amber-300">Mercado Originario Digital — Nayarit</p>
          <p className="text-xs text-amber-600 mt-0.5">Artesanas y artesanos de Nayarit vendiendo al mundo desde su comunidad. Sin intermediarios.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {[
          { num: '01', titulo: 'El artesano publica',   desc: 'Sube fotos, precio y descripción desde su celular. En español, wixárika o náayeri.', icon: Package, color: 'text-amber-400' },
          { num: '02', titulo: 'El cliente compra',     desc: 'Pago en línea o en OXXO. El artesano recibe el dinero directo a su cuenta o tarjeta.', icon: ShoppingBag, color: 'text-blue-400' },
          { num: '03', titulo: 'El arte llega al mundo',desc: 'Envío desde la comunidad con guía prepagada. Rastreo en tiempo real para ambas partes.', icon: TrendingUp, color: 'text-emerald-400' },
        ].map(s => {
          const Icon = s.icon;
          return (
            ...
            {ARTESANOS.map(a => (
              <div key={a.id} className="flex items-center gap-3 px-5 py-3.5 hover:bg-slate-800/30 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[9px] text-slate-600 mt-0.5">{a.comunidad}, Nayarit</p>
                </div>
                <div className="text-right shrink-0">
      ...
      <p className="text-[10px] text-slate-700 text-center font-mono">
        Mercado Originario Digital · ConnectX · Nayarit Digital · Impulsando la economía comunitaria
      </p>
    </div>
  );
}
...
const TABS: { id: PatrimonioTab; label: string; icon: React.ElementType }[] = [
...
export function PatrimonioView() {
  const [tab, setTab] = useState<PatrimonioTab>('lenguas');
